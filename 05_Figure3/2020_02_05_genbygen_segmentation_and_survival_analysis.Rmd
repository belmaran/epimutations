...
DESeq analysis to remove batch variation (early vs late, plus B1-B4 samples)
...
# Segmentation of counts data into low and high 22G counts

In this part of the code, I apply k-means clustering to the counts distributions to obtain binarized "high"-"low" small RNA count states for each gene.
...
# Epimutation data retrieved from the segmented data

In this part of the code, I read the high-low small RNA state traces in order to retrieve the duration of high and low states from the consecutive generation data. In the end a table is generated where each row is an epimutation event, recording whether it is an increase or a decrease in small RNAs, its duration, the line in which it started, whether it is a complete or an incomplete event (reaching the end of the experiment therefore of unknown total duration).
...
# plotting the distribution of event duration 
...
We apply survival analysis concepts to the total set of epimutations in order to obtain overall estimates of duration taking into account the fact that some epimutations reach the end of the lineage. The results obtained from lineages A and B are very consistent.
