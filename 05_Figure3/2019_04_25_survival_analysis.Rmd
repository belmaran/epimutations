---
title: "2019_04_25_survival_analysis"
author: "Toni Beltran"
date: "25/04/2019"
...
duration_A<-read.table("epimut_duration_batchremoved_kmeans_linA_JOINT_p1e-4_FCdata_noiso.txt",sep="\t",header=TRUE)
duration_B<-read.table("epimut_duration_batchremoved_kmeans_linB_JOINT_p1e-4_FCdata_noiso.txt",sep="\t",header=TRUE)
...
Here I transform the 22G fold change data in many ways (absolute difference in 22G normalized counts (difference), fold change, log fold change etcetera) to test whether these correlate in any way with epimutation duration.
...
Similar analysis as above but for mRNA data.
...
Similar to the 22G data above but considering only gains of 22Gs.
...
# Testing the correlation of different gene features with epimutation duration
...
overall_mRNA_abundance<-read.table("../02_Normalised_counts/RNAseq_DEseqnorm.txt")
mRNA_ab<-data.frame(mRNA=apply(overall_mRNA_abundance,MARGIN=1,FUN=mean))
duration_both_geneinfo<-merge(duration_both_geneinfo,mRNA_ab,by.x="ID",by.y = 0)
duration_both_geneinfo$mRNA<-as.numeric(duration_both_geneinfo$mRNA)
duration_both_geneinfo$mRNA_log2<-log2(duration_both_geneinfo$mRNA+1)

surv_geneinfo<-Surv(time=duration_both_geneinfo$duration,
                    event=duration_both_geneinfo$censored)

fit.coxph <- coxph(surv_geneinfo ~ mRNA, data = duration_both_geneinfo)
summary(fit.coxph)
fit.coxph <- coxph(surv_geneinfo ~ mRNA_log2, data = duration_both_geneinfo)
summary(fit.coxph)

hist(duration_both_geneinfo$mRNA_log2,breaks=100)
#bins <5, 5-10, >10

duration_both_geneinfo$mRNA_log2_groups<-rep("<5",nrow(duration_both_geneinfo))
duration_both_geneinfo[which(duration_both_geneinfo$mRNA_log2>5 & duration_both_geneinfo$mRNA_log2<10), "mRNA_log2_groups"]<-"5-10"
duration_both_geneinfo[which(duration_both_geneinfo$mRNA_log2>9.9999999), "mRNA_log2_groups"]<-">10"

surv_geneinfo<-Surv(time=duration_both_geneinfo$duration,
                    event=duration_both_geneinfo$censored)
fit_geneinfo<-survfit(surv_geneinfo~mRNA_log2_groups,data=duration_both_geneinfo)
summary(fit_geneinfo)
ggsurvplot(fit_geneinfo, data=duration_both_geneinfo,pval = TRUE)


pdf("epimutable_genes_expression.pdf")
boxplot(unique(duration_both_geneinfo[which(duration_both_geneinfo$csr1_targets==TRUE),c("ID","mRNA_log2")])[,"mRNA_log2"],
unique(duration_both_geneinfo[which(duration_both_geneinfo$hrde1_targets==TRUE),c("ID","mRNA_log2")])[,"mRNA_log2"],
unique(duration_both_geneinfo[which(duration_both_geneinfo$wago1_targets==TRUE),c("ID","mRNA_log2")][,"mRNA_log2"]),
unique(duration_both_geneinfo[which(duration_both_geneinfo$piRNA_targets==TRUE),c("ID","mRNA_log2")][,"mRNA_log2"]),
names=c("csr-1","hrde-1","wago-1","piRNA"),main="epimutable genes",ylab="log2(mRNA normalized counts+1)")
dev.off()
