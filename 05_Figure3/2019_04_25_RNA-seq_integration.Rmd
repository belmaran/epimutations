---
title: "2019_04_15_RNA-seq integration"
author: "Toni Beltran"
date: "15/04/2019"
...
setwd("/Users/ab6415/Documents/Work_laptop_stuff/Bioinformatics/Analysis/EpiMALs_PAPER_ANALYSES/05_Figure3/")
...
## Wilcoxon rank sum test on the RNA-seq counts of samples with high and low small RNAs (lineage A and B samples)
...
kmeans_clusters_byrow_all<-kmeans_clusters_byrow_all[-which(apply(kmeans_clusters_byrow_all[,1:24],MARGIN=1,FUN=sum)==24),]

for (gene in rownames(kmeans_clusters_byrow_all)){
  if (gene %in% rownames(RNAseq_DEseqnorm)){
  rnaseq<-as.numeric(RNAseq_DEseqnorm[gene,1:24])
  ones<-rnaseq[which(kmeans_clusters_byrow_all[gene,1:24]==1)]
  twos<-rnaseq[which(kmeans_clusters_byrow_all[gene,1:24]==2)]
  test<-wilcox.test(as.numeric(ones),as.numeric(twos))
  ...
  ups_mRNA<-c(ups_mRNA,mean(as.numeric(twos)))
  downs_mRNA<-c(downs_mRNA,mean(as.numeric(ones)))
  
  ttg<-ttg_DEseqnorm[gene,1:24]
  ttg_ones<-ttg[which(kmeans_clusters_byrow_all[gene,1:24]==1)]
  ttg_twos<-ttg[which(kmeans_clusters_byrow_all[gene,1:24]==2)]
  test<-wilcox.test(as.numeric(ttg_ones),as.numeric(ttg_twos))
  ...
  ups_22Gs<-c(ups_22Gs,mean(as.numeric(ttg_twos)))
  downs_22Gs<-c(downs_22Gs,mean(as.numeric(ttg_ones)))
...
wilcoxon_data<-data.frame(pvals=wilcox_pvalues,logfc=log2_foldchanges_wilcox,
                          ...
                          ups_22Gs=ups_22Gs,downs_22Gs=downs_22Gs,ups_mRNA=ups_mRNA,downs_mRNA=downs_mRNA)
...
wilcoxon_data[wilcoxon_data$pvals<=0.1,"sig"]<-1; wilcoxon_data$sig<-as.factor(wilcoxon_data$sig)

ggplot(wilcoxon_data)+geom_point(aes(x=logfc,y=logfc_22G,color=sig))+xlab("log2 fold change in mRNA levels")+ylab("log2 fold change in 22Gs")

```


## Wilcoxon rank sum test on the RNA-seq counts of samples with high and low small RNAs (all samples, including the consecutive generation experiment and the 25th and 100th generation experiment)
...
kmeans_clusters_byrow_all_gens25_100<-read.table("../03_Figure1/k-means_segmentation_epimutable_genes.txt")
...
ggplot(wilcoxon_data)+geom_point(aes(x=logfc,y=-log(padj,10)))+geom_hline(yintercept=-log10(0.2),col="lightblue")

ggplot(wilcoxon_data)+geom_point(aes(x=logfc,y=logfc_22G,color=sig))+xlab("log2 fold change in mRNA levels")+ylab("log2 fold change in 22Gs")+theme_classic()+scale_color_manual(values = c("#EE756E","#47A2CC"))
ggsave("log2FC_mRNA_vs_22Gs_epimutations_p1e-4.pdf",dpi="retina")

write.table(wilcoxon_data,file="RNAseq_tests_wilcoxon.txt")


wilcoxon_data$totalchange_22Gs<-wilcoxon_data$ups_22Gs-wilcoxon_data$downs_22Gs
wilcoxon_data$totalchange_mRNA<-wilcoxon_data$ups_mRNA-wilcoxon_data$downs_mRNA
wilcoxon_data$totalchange_22Gs_log<-log2(abs(wilcoxon_data$totalchange_22Gs))
wilcoxon_data$totalchange_mRNA_log<-log2(abs(wilcoxon_data$totalchange_mRNA))


ggplot(wilcoxon_data)+geom_point(aes(x=totalchange_mRNA,y=totalchange_22Gs,color=sig))+xlab("total change in mRNA levels")+ylab("total change in 22Gs")
ggplot(wilcoxon_data)+geom_point(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log,color=sig))+xlab("total change in mRNA levels")+ylab("total change in 22Gs")+scale_color_manual(values = c("#EE756E","#47A2CC"))+
  theme_classic()+geom_smooth(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log),method='lm')
ggsave("logscale_absolutechanges_in_mRNAs_vs_22Gs.pdf",device = "pdf")
cor.test(wilcoxon_data$totalchange_22Gs_log,wilcoxon_data$totalchange_mRNA_log)
cor(wilcoxon_data$totalchange_22Gs_log,wilcoxon_data$totalchange_mRNA_log)

wilcoxon_data_22G_pos<-wilcoxon_data[which(wilcoxon_data$logfc>0),]
ggplot(wilcoxon_data_22G_pos)+geom_point(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log,color=sig))+xlab("total change in mRNA levels")+ylab("total change in 22Gs")+scale_color_manual(values = c("#EE756E","#47A2CC"))+
  theme_classic()+geom_smooth(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log),method='lm')
ggsave("logscale_absolutechanges_in_mRNAs_vs_22Gs_poscor.pdf",device = "pdf")
cor.test(wilcoxon_data_22G_pos$totalchange_22Gs_log,wilcoxon_data_22G_pos$totalchange_mRNA_log)
cor(wilcoxon_data_22G_pos$totalchange_22Gs_log,wilcoxon_data_22G_pos$totalchange_mRNA_log)

wilcoxon_data_22G_neg<-wilcoxon_data[which(wilcoxon_data$logfc<0),]
ggplot(wilcoxon_data_22G_neg)+geom_point(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log,color=sig))+xlab("total change in mRNA levels")+ylab("total change in 22Gs")+scale_color_manual(values = c("#EE756E","#47A2CC"))+
  theme_classic()+geom_smooth(aes(x=totalchange_mRNA_log,y=totalchange_22Gs_log),method='lm')
ggsave("logscale_absolutechanges_in_mRNAs_vs_22Gs_negcor.pdf",device = "pdf")
cor.test(wilcoxon_data_22G_neg$totalchange_22Gs_log,wilcoxon_data_22G_neg$totalchange_mRNA_log)
cor(wilcoxon_data_22G_neg$totalchange_22Gs_log,wilcoxon_data_22G_neg$totalchange_mRNA_log)



```

## Chromatin locations of genes on the plot
...
## Testing whether the set of epimutable genes is enriched in changes at the mRNA level

We repeat the same analysis for random sets of genes of the same size as the epimutable set. Background genes are first filtered to have at least the minimum 22G abundance required to be called as epimutable. These are then segmented, and differences in mRNA levels between high and low small RNA groups are assessed in the same way as above (Wilcoxon rank sum test).
...
This code calculates the fold changes in 22G-RNA and mRNA levels for each epimutation event and adds it to the epimutations summary table generated after the segmentation analysis - this is for downstream testing of the relationship between epimutation duration and the magnitude and direction of changes in 22G-RNA levels and mRNA levels.
...
#add average mRNA fold change
epimut_duration_kmeans_A$overall_mRNA_fc<-rep(0,nrow(epimut_duration_kmeans_A))
for (i in seq(nrow(epimut_duration_kmeans_A))){
  gene<-as.character(epimut_duration_kmeans_A$ID[i])
  if (gene %in% rownames(wilcoxon_data)){
  logfc<-wilcoxon_data[which(rownames(wilcoxon_data)==gene),"logfc"]}
  else {logfc<-NA}
  epimut_duration_kmeans_A$overall_mRNA_fc[i]<-logfc
}

epimut_duration_kmeans_B$overall_mRNA_fc<-rep(0,nrow(epimut_duration_kmeans_B))
for (i in seq(nrow(epimut_duration_kmeans_B))){
  gene<-as.character(epimut_duration_kmeans_B$ID[i])
  if (gene %in% rownames(wilcoxon_data)){
  logfc<-wilcoxon_data[which(rownames(wilcoxon_data)==gene),"logfc"]}
  else {logfc<-NA}
  epimut_duration_kmeans_B$overall_mRNA_fc[i]<-logfc
}
...
## Same for HMM segmentation data
